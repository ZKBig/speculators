...
"""Launch XPress online training on the k8s job framework.

A copy of k8s_dflash2_launch.py with XPress's flags. It is a copy rather than a
shared launcher on purpose: those files are what running jobs re-clone on
restart, and a shared one would put their next restart at the mercy of an edit
made for this one. Merge them once the runs are done.

XPress trains from scratch here rather than warm-starting from a converted
DFlash backbone: no such backbone is published for Qwen3-4B, and the DFlash2 and
DSpark runs this is compared against start cold too.
...
TRAIN_PORT = int(env("XP4_TRAIN_PORT", str(MASTER_PORT + 1)))

MODEL = env("XP4_MODEL", "Qwen/Qwen3-4B")
OUT_ROOT = Path(env("XP4_OUT_ROOT", "/gpfs/zwang33/xpress4b"))
OUTPUT_DIR = OUT_ROOT / env("XP4_RUN_DIR", "xpress_qwen3_4b_8spec")
DATA_DIR = Path(env("XP4_DATA_DIR", str(OUTPUT_DIR / "data")))
VAL_DATA_DIR = env("XP4_VAL_DATA_DIR", "")
...
TRAIN_SRC = env("XP4_TRAIN_SRC", "")
MAX_SAMPLES = env("XP4_MAX_SAMPLES", "")
PREP_WORKERS = env("XP4_PREP_WORKERS", "32")

SEQ_LENGTH = env("XP4_SEQ_LENGTH", "8192")
TARGET_LAYER_IDS = env("XP4_TARGET_LAYER_IDS", "1 9 17 25 33").split()
...
NUM_JACOBI_PASSES = env("XP4_NUM_JACOBI_PASSES", "6")
# Free-running Jacobi rounds during training: each round is conditioned on the
# previous round's argmax, which is the recurrence inference performs.
CONSISTENCY_PASSES = env("XP4_CONSISTENCY_PASSES", "3")
CONSISTENCY_WEIGHT = env("XP4_CONSISTENCY_WEIGHT", "0.3")
# Anchors the co-trained backbone so its standalone drafting does not erode;
# decays linearly to the floor over the run.
BASE_ANCHOR_WEIGHT = env("XP4_BASE_ANCHOR_WEIGHT", "0.6")
BASE_ANCHOR_FLOOR = env("XP4_BASE_ANCHOR_FLOOR", "0.2")
DECAYED_LOSS_NORM = env("XP4_DECAYED_LOSS_NORM", "1")
LOSS_FN = env("XP4_LOSS_FN", '{"ce": 0.1, "tv": 0.9}')
DECAY_GAMMA = env("XP4_DECAY_GAMMA", "4.0")
EPOCHS = env("XP4_EPOCHS", "1")
LR = env("XP4_LR", "6e-4")
WEIGHT_DECAY = env("XP4_WEIGHT_DECAY", "0.0")
WARMUP_RATIO = env("XP4_WARMUP_RATIO", "0.04")
CHECKPOINT_FREQ = env("XP4_CHECKPOINT_FREQ", "0.1")
LOG_FREQ = env("XP4_LOG_FREQ", "50")
RUN_NAME = env("XP4_RUN_NAME", "xpress-qwen3-4b-8spec")
...
PREP_ONLY = env("XP4_PREP_ONLY", "")

VLLM_PORT = int(env("XP4_VLLM_PORT", "8300"))
...
VLLM_HOST = env("XP4_VLLM_HOST", "127.0.0.1")
VLLM_VENV = Path(env("XP4_VLLM_VENV", "/gpfs/zwang33/venv_vllm"))
VLLM_PY = Path(env("XP4_VLLM_PY", str(VLLM_VENV / "bin" / "python")))
...
def child_env(**extra: str) -> dict:
    ...
    # process before.
    e["TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC"] = env("XP4_NCCL_HEARTBEAT_SEC", "3600")
    e.update(extra)
    return e
...
def check_inputs() -> None:
    ...
    if not data_is_prepared() and not TRAIN_SRC:
        sys.exit(
            ...
            "dataset_info.json) and XP4_TRAIN_SRC is unset, so there is nothing to "
            "build it from. Online training generates only the HIDDEN STATES on the "
            "fly; the tokenized data still has to exist. Set XP4_TRAIN_SRC to a "
            'JSONL path or an "hf:org/dataset" spec, or point XP4_DATA_DIR at an '
            "already-prepared corpus."
        )
    if not VLLM_PY.exists():
        sys.exit(
            f"[r{RANK}] FATAL - verifier interpreter {VLLM_PY} not found; "
            "set XP4_VLLM_VENV to a venv that has vllm installed."
        )
...
def serve_vllm_until_done() -> int:
    ...
    spool = Path(env("XP4_HIDDEN_STATES_DIR", "/tmp/hidden_states_xpress4b"))
    if spool.exists():
    ...
    e = strip_dist(
        child_env(
            ...
            VLLM_HOST_IP=env("XP4_VLLM_HOST_IP", "127.0.0.1"),
            HOST_IP=env("XP4_VLLM_HOST_IP", "127.0.0.1"),
            ...
            VLLM_ATTENTION_BACKEND=env("XP4_VLLM_ATTN_BACKEND", "FLASH_ATTN"),
        )
    )
    ...
    #   XP4_VLLM_V1_MP=0     run EngineCore in-process (no loopback TCPStore)
    #   XP4_VLLM_USE_LIBUV=0 legacy store backend; libuv's listener can fail
    #                        silently in a restricted container
    for _var, _key in (
        ("XP4_VLLM_V1_MP", "VLLM_ENABLE_V1_MULTIPROCESSING"),
        ("XP4_VLLM_USE_LIBUV", "USE_LIBUV"),
    ):
        _val = env(_var, "")
        if _val:
            e[_key] = _val
    ...
    servers = env("XP4_VLLM_API_SERVERS", "")
    if servers:
        cmd += ["--api-server-count", servers]
    bind = env("XP4_VLLM_BIND", "")
    if bind:
        cmd += ["--host", bind]
    ...
    if data_is_prepared():
        ...
        if STAMP.exists() and have != want:
            sys.exit(
                ...
                f"  want: {want}\n  have: {have}\n"
                "Point XP4_DATA_DIR somewhere else rather than overwriting it."
            )
...
def wait_for_prep(timeout_s: int | None = None) -> None:
    ...
    timeout_s = timeout_s or int(env("XP4_PREP_TIMEOUT_S", "43200"))
    print(
...
def train() -> int:
    ...
    if DECAYED_LOSS_NORM == "1":
        cmd += ["--decayed-loss-norm"]
    if VAL_DATA_DIR:
